create table if not exists game_items (
    ...
    addHp integer,
    addDmg integer,
    resultAction boolean not null default true
);

insert into game_items (name , addHp) values
('Зелье Здоровья', 10);
...
create table if not exists inventories (
    ...
    char_id integer,
  	...
  	constraint inventories_items_id_fkey foreign key (char_id) references users_char (id)
);
